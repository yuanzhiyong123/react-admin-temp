...
const getContentListByConfig = (
  ...
  configList.forEach((configItem) => {
    ...
    if (typeof configItem === "object") {
      ...
      result = [value, dataItem[key], seriesData[key], paramsItem[key]].filter(
 ...
 * @param {*} tooltips
 */
const tooltipTitleHandler = ({ params = [], source = [], tooltips = {} }) => {
 ...
 * @param {*} nameMapper
 */
const tooltipHeaderHandler = ({ tooltips = {}, nameMapper = {} }) => {
  const { body } = tooltips;
  const headerContent =
    body &&
    body.map((key) => {
 ...
 * @param {*} nameMapper
 */
const tooltipBodyHandler = ({ params = [], source = [], tooltips = {} }) => {
  const { body = [] } = tooltips;
 ...
 * @param {*} nameMapper
 */
const tooltipDomHandler = (params, source, tooltips, nameMapper) => {
